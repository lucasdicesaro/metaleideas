<?php
	...
	while($row = mysql_fetch_array($result)) {
		echo '<li class="ui-state-default ui-corner-top ui-corner-bottom" id="categoria_' . $row['categoria_id'] . '"><span class="ui-icon ui-icon-arrowthick-2-n-s fl_left list-icon"></span>' . utf8_encode($row['nombre']) . '<span onclick="borrar(' . $row['categoria_id'] . ')" class="cursor-pointer ui-icon ui-icon-close fl_right" title="Eliminar categoria"></span><span onclick="buscarproductosxcategoria(' . $row['categoria_id'] . ')" class="cursor-pointer ui-icon ui-icon-search fl_right" title="Ver los productos de la categoría"></span></li>';																			
	}
